...
static void verifyMatmul() {
  ...
  MemRef<float, 2> dyno_buffer({M, N}, 0);

  runKernel(_mlir_ciface_matmul, &input1, &input2, &dyno_buffer);
  runKernel(naive_matmul, &input1, &input2, &naive_buffer);

  dyno_buffer.verify(naive_buffer, "Matmul", 0.0001);

  // print first 10 elements
  for (int i = 0; i < 10; i++) {
    std::cout << "Index " << i << ":\tNaive=" << std::setprecision(10)
              << naive_buffer[i] << " Dyno=" << std::setprecision(10)
              << dyno_buffer[i] << std::endl;
  }
}

BENCHMARK_CAPTURE(runBenchmark, dyno, _mlir_ciface_matmul)
