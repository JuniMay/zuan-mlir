...
static void runKernel(kernel_ptr_t kernel, uint32_t n_elements, float alpha,
                      float *vec_a, float *vec_b, float *vec_c) {
  uint32_t gridX = std::ceil(n_elements / static_cast<float>(BLOCK_SIZE));
  launch_kernel(gridX, 1, 1, kernel, alpha, vec_a, vec_b, vec_c, n_elements);
}
...
static void runBenchmark(benchmark::State &state, kernel_ptr_t kernel) {
  ...
  for (auto _ : state) {
    runKernel(kernel, n_elements, ALPHA, vec_a.data(), vec_b.data(),
              vec_c.data());
  }
}
...
void verify() {
  ...
  runKernel(kernel_scalar_wrapper, N, ALPHA, vec_a.data(), vec_b.data(),
            vec_c_scalar.data());
  runKernel(kernel_triton_cpu, N, ALPHA, vec_a.data(), vec_b.data(),
            vec_c_triton_cpu.data());
  runKernel(kernel_zuan_wrapper, N, ALPHA, vec_a.data(), vec_b.data(),
            vec_c_zuan.data());

  verify(vec_c_scalar.data(), vec_c_triton_cpu.data(), N, "SAXPY Triton CPU");
  verify(vec_c_scalar.data(), vec_c_zuan.data(), N, "SAXPY Zuan");
}
