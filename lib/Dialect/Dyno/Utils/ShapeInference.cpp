//===- ShapeInference.cpp - Local shape reification for Dyno ---*- C++ -*-===//
//
...
reifyBlockArgumentShape(OpBuilder &builder, BlockArgument arg) {
  ...
  if (auto forOp = dyn_cast<scf::ForOp>(arg.getOwner()->getParentOp())) {
    ...
    return reifyDynoShape(builder, forOp.getInitArgs()[arg.getArgNumber() - 1]);
  }

  if (auto whileOp = dyn_cast<scf::WhileOp>(arg.getOwner()->getParentOp())) {
    return reifyDynoShape(builder, whileOp.getInits()[arg.getArgNumber()]);
  }
...
reifySplatShape(OpBuilder &builder, SplatOp splatOp) {
  ...
  if (isa<TileType>(splatOp.getValue().getType())) {
    auto sourceShape = reifyDynoShape(builder, splatOp.getValue());
...
reifyMaskShape(OpBuilder &builder, MaskOp maskOp, unsigned resultNumber) {
  ...
  if (auto maskedoff = maskOp.getMaskedoff()) {
    return reifyDynoShape(builder, maskedoff);
  }
  return reifyDynoShape(builder, maskOp.getMask());
}
...
reifyGatherShape(OpBuilder &builder, GatherOp gatherOp) {
  ...
  auto shape = reifyDynoShape(builder, gatherOp.getIndices().front());
  if (failed(shape)) {
    return failure();
  }
...
reifySCFForResultShape(OpBuilder &builder, OpResult result) {
  ...
  return reifyDynoShape(builder, forOp.getInitArgs()[result.getResultNumber()]);
}

static FailureOr<SmallVector<OpFoldResult>>
reifySCFWhileResultShape(OpBuilder &builder, OpResult result) {
  auto whileOp = cast<scf::WhileOp>(result.getOwner());
  return reifyDynoShape(builder, whileOp.getInits()[result.getResultNumber()]);
}
...
reifyOpResultShape(OpBuilder &builder, OpResult result) {
  ...
  return TypeSwitch<Operation *, FailureOr<SmallVector<OpFoldResult>>>(op)
      ...
      .Default([&](Operation *unknownOp) -> FailureOr<SmallVector<OpFoldResult>> {
        ...
        auto shape = reifyDynoShape(builder, *forwarded);
        if (failed(shape)) {
          return failure();
        }
        overlayStaticShape(builder, cast<TileType>(result.getType()), *shape);
...
FailureOr<SmallVector<OpFoldResult>> reifyDynoShape(OpBuilder &builder,
                                                    Value value) {
...
FailureOr<OpFoldResult> reifyDynoDim(OpBuilder &builder, Value value,
                                     unsigned dim) {
  auto tileType = dyn_cast<TileType>(value.getType());
...
LogicalResult resolveDimUsersOfResult(OpResult result,
  ...
  for (OpOperand &use : llvm::make_early_inc_range(result.getUses())) {
    // Only direct `dyno.dim` users need special handling here. Any other live
    ...
    // direct `dyno.dim` users are rewritten while the original tile semantics
    // are still available.
    auto reified = reifyDynoDim(rewriter, result, dimOp.getDim());
    if (failed(reified)) {
...
std::optional<int64_t> getConstantDynoIntValue(OpFoldResult ofr) {
