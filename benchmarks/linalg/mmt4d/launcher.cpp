...
static void verifyMmt4D() {
  ...
  auto [input1, input2] = initializeData(M, N, K, M0, N0, K0);
  MemRef<float, 4> scalar({M, N, M0, N0}, 0);
  runKernel(_mlir_ciface_kernel_scalar, &input1, &input2, &scalar);

  MemRef<float, 4> dyno_8_4({M, N, M0, N0}, 0);
  runKernel(_mlir_ciface_kernel_dyno_8_4, &input1, &input2, &dyno_8_4);
  MemRef<float, 4> dyno_16_2({M, N, M0, N0}, 0);
  runKernel(_mlir_ciface_kernel_dyno_16_2, &input1, &input2, &dyno_16_2);

  scalar.verify(dyno_16_2, "mmt4d-dyno-16-2", 0.0001);
  scalar.verify(dyno_8_4, "mmt4d-dyno-8-4", 0.0001);
  ...
  scalar.verify(transform_32_4, "mmt4d-transform-32-4", 0.0001);
  scalar.verify(transform_64_2, "mmt4d-transform-64-2", 0.0001);

  // print first 10 elements
  for (int i = 0; i < 10; i++) {
    std::cerr << "Index " << i << std::setprecision(10)
              << ": scalar=" << scalar[i] << "\tdyno-16-2=" << dyno_16_2[i]
              << "\tdyno-8-4=" << dyno_8_4[i]
